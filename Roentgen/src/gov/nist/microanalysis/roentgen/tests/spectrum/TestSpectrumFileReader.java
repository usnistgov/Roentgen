...
public class TestSpectrumFileReader {
   ...
   private File makeFile(final String resName)
         throws IOException {
   ...
   @Test
   public void testEMSAFileReader()
         throws Exception {
      ...
      try {
         ...
         if(false) {
            final LineshapeCalibration lsc = new LineshapeCalibration.Gaussian(130.0, LineshapeCalibration.Gaussian.SDD_EV_PER_EH);
            final AdaptiveTophatFilter atf = new AdaptiveTophatFilter(spec.size(), spec.getEnergyCalibration(), lsc);
            final Pair<RealVector, RealMatrix> res = atf.evaluate(spec.getData());
            final RealVector vals = res.getFirst();
            final AdaptiveGaussianFilter agf = new AdaptiveGaussianFilter(spec.size(), spec.getEnergyCalibration(), lsc);
   ...
   @Test
   public void testEMSAFileReader2()
         throws Exception {
      final SpectrumFileReader sfr = Globals.getSpectrumFileReader();
