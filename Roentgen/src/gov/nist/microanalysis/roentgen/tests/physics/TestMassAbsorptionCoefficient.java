...
public class TestMassAbsorptionCoefficient {
   ...
   public void testMac(final Composition comp, final Element elm, final XRayTransition tr, final double mac, final double uMac) {
   ...
   public void testMultiMaterial()
      ...
      final Composition[] mfs = new Composition[] {
         mf1,
         mf2,
         mf3,
         mf4,
      ...
      minCov.set(1.0);
      final UncertainValues mc = UncertainValues.propagateMC(pr.getSecond(), UncertainValues.forceMinCovariance(pr.getFirst(), minCov), 16000);
      if(REPORT) {
      ...
      for(int p = 0; p < uv.getDimension(); ++p) {
         ...
         for(int q = p + 1; q < uv.getDimension(); ++q) {
            ...
            Assert.assertEquals(uv.getCovariance(p, q), mc.getCovariance(p, q), 0.05
                  * Math.sqrt(uv.getVariance(p) * uv.getVariance(q)));
         }
      }
