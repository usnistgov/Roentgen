...
public class IonizationCrossSection
   ...
   private static class Implementation {
      ...
      private final double[][][] mA; // nominally [100][9][5]
      private final double[][] mBe; // nominally [100][9]
      private final double[][] mAnlj; // nominally [100][9]
      private final double[][][] mG; // nominally [100][9][4]
       ...
       */
      public double compute(final int atomicNumber, final int shellIdx, final double edgeEnergy, final double energy) {
         double xione = 0.0;
         if((edgeEnergy >= 1.0) && (energy > edgeEnergy)) {
            ...
            if(u <= 16.0) {
               ...
               assert shellIdx < mBe[atomicNumber].length : shellIdx + " >= " + mBe[atomicNumber].length + " for " + " for Z="
                     + atomicNumber + " shell=" + shellIdx;
               assert shellIdx < mG[atomicNumber].length : shellIdx + ">= " + mG[atomicNumber].length + " for " + shellIdx;
      ...
      public double compute(final AtomicShell shell, final double energy) {
         return compute(shell.getElement().getAtomicNumber(), shell.getShell().ordinal(), shell.getEdgeEnergy(), energy);
      }
   ...
   private static final SimplyLazy<Implementation> IMPLEMENTATION = new SimplyLazy<Implementation>() {

      @Override
      protected Implementation initialize() {
         try {
   ...
   private final int mShellIndex;

   public IonizationCrossSection(final AtomicShell shell) {
      final int iz = shell.getElement().getAtomicNumber();
      assert ((iz >= Element.Hydrogen.getAtomicNumber())
            && (iz <= Element.Einsteinium.getAtomicNumber())) : "Unsupported element in BoteSalvatCrossSection.computeShell: "
      ...
      mShell = shell;
      mAtomicNumber = shell.getElement().getAtomicNumber();
      mEdgeEnergy = mShell.getEdgeEnergy();
      mShellIndex = mShell.getShell().ordinal();
   }
   ...
   public static double value(final AtomicShell shell, final double energy) {
      final Implementation impl = IMPLEMENTATION.get();
      assert impl != null;
      return impl != null ? impl.compute(shell, energy) : 0.0;
   }
   ...
   public double value(final double energy) {
      final Implementation impl = IMPLEMENTATION.get();
      assert impl != null;
      return impl != null ? impl.compute(mAtomicNumber, mShellIndex, mEdgeEnergy, energy) : 0.0;
   }

   public static boolean isSupported(final AtomicShell sh) {
      final Implementation impl = IMPLEMENTATION.get();
      assert impl != null;
      return impl != null ? impl.isSupported(sh) : false;
   }
   ...
   public static Set<AtomicShell> getSupportedAtomicShells(final Element elm) {
      final Implementation impl = IMPLEMENTATION.get();
      assert impl != null;
      return impl != null ? impl.getSupportedAtomicShells(elm) : Collections.emptySet();
   }
