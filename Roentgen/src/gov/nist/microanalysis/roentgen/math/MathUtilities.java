...
public class MathUtilities {
	...
	/**
	 * Parses "number" to a Double, "(number&plusmn;number)" / "number+-number" to
	 * an UncertainValue and "[number,number]" / "[number-number]" to a Interval.
	 *
	 * @param number
	 * @return Number
	 */
	...
	public static String toHTML_Image(final RealMatrix rm, final String caption, final File base, final String dir)
		...
		final IDoubleAsColor colorize = new PositiveNegativeAsColor(min, Color.red, max, Color.green, Color.yellow,
				0.5);
	...
	public static class LinearDoubleAsColor implements IDoubleAsColor {
		final int[] mMinRgb;
		final int[] mMaxRgb;
		final double mMin;
		final double mMax;
	...
	public static IToHTML toHTML(final DescriptiveStatistics desc, final NumberFormat bnf) {
		...
		class DescStatTable implements IToHTML {
			...
			public String toHTML(final Mode mode) {
				...
				items.add(Arrays.asList("Standard Deviation (&sigma;)",
						mFormat.format(mDescStat.getStandardDeviation()),
						mFormat.format((100.0 * mDescStat.getStandardDeviation()) / mDescStat.getMean()) + " %"));
				if ((mode == Mode.NORMAL) || (mode == Mode.VERBOSE)) {
					...
					if (mode == Mode.VERBOSE) {
						final int[] pcts = new int[] { 1, 5, 10, 25, 50, 75, 90, 95, 99 };
						for (final int pct : pcts)
							items.add(Arrays.asList(HTML.asOrdinalHTML(pct) + " percentile",
									mFormat.format(mDescStat.getPercentile(pct)), ""));
					}
				}
