...
public class NamedMultivariateJacobian
   ...
   private NamedMultivariateJacobian(final List<? extends Object> inp, final List<? extends Object> out, final RealVector pt, final Pair<RealVector, RealMatrix> result) {
      super(inp, out);
      mPoint = pt;
      mResult = result;
   }
   ...
   public static NamedMultivariateJacobian computeDelta(final NamedMultivariateJacobianFunction nmjf, final UncertainValues uv, final double sc) {
      ...
      for(int c = 0; c < nmjf.getInputDimension(); ++c) {
         ...
         pt.setEntry(c, pt.getEntry(c) + sc * Math.sqrt(uv.getVariance(c)));
         final RealVector output = nmjf.evaluate(pt).getFirst();
         for(int r = 0; r < nmjf.getOutputDimension(); ++r) {
            rm.setEntry(r, c, (output.getEntry(r) - vals.getEntry(r)) / (pt.getEntry(c) - inp.getEntry(c)));
         }
      }
   ...
   /**
    * Extract a subset of the output values and the associated Jacobian
    * elements.
    *
    * @param output
   ...
   public String toHTML(final Mode mode, final BasicNumberFormat nf) {
      ...
      switch(mode) {
         ...
         default: {
            ...
            res.addRow(Table.td(valt.toHTML(Mode.NORMAL)), Table.tdc("<bold>J</bold> ="), Table.td(jact.toHTML(Mode.NORMAL)));
