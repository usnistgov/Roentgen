...
final public class UncertainValue
    ...
    * <code>dv</code>. The uncertainty is given a default name "DEFAULTXXX"=
    * where XXX is an auto-incrementing index to ensure every unnamed
    * uncertainty is considered independent.
    *
    * @param v The value
   ...
   public Map<Object, Double> getComponents() {
      final Map<Object, Double> res = new TreeMap<>();
   ...
   public Set<Object> getComponentNames() {
      final Set<Object> res = new TreeSet<>();
      for(final Sigma s : mSigmas)
         res.add(s.mName);
    ...
    */
   static public Number weightedMean(final Collection<? extends UncertainValue> cuv)
         throws Exception {
      ...
      for(final UncertainValue uv : cuv) {
         ...
         if(Double.isNaN(ivar))
            throw new Exception("Unable to compute the weighted mean when one or more datapoints have zero uncertainty.");
         varSum += ivar;
         sum += ivar * uv.doubleValue();
      }
      final double iVarSum = 1.0 / varSum;
      return Double.isNaN(iVarSum) ? UncertainValue.NaN : new UncertainValue(sum / varSum, "WM", Math.sqrt(1.0 / varSum));
   }
