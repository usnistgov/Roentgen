...
public class MultiStepNamedMultivariateJacobianFunction
   ...
   /**
    * Steps in order from inner-most to outer-most. The output of the inner
    * steps become the input of the subsequent outer.
    */
   ...
   /**
    * A list containing the tags associated with all required input values that
    * are not computed as output values. (Values for these tags must be provided
    ...
    * steps in order, where the output values for earlier steps become available
    * as input values to later steps.
    *
    * @param steps
    * @throws ArgumentException
    */
   public MultiStepNamedMultivariateJacobianFunction(final String name, final List<NamedMultivariateJacobianFunction> steps)
         throws ArgumentException {
    ...
    *
    * @param tag A tag associated with an NamedMultivariateJacobianFunction
    *           input
    * @returns int The index of the NamedMultivariateJacobianFunction in which
    *          this tag is requested as input.
    */
   ...
   /**
    * Returns an array consisting of the tags associated with the m output
    * random variables.
    *
    * @return List&lt;Object&gt;
    */
   ...
   /**
    * A mechanism for optimizing the calculation by keeping a subset of the
    * outputs at each step. <code>finalOutput</code> specifies which outputs to
    * keep regardless of whether they are required by a subsequent step. At each
    * step in the calculation determine which outputs are required by subsequent
    * steps and and keep only these outputs.
    *
    * @param finalOutputs
    */
      ...
      switch(mode) {
         ...
         case NORMAL: {
            ...
            for(int i = 0; i < out.size(); ++i)
               tbl.addRow(Table.td("Output " + Integer.toString(i + 1)), Table.td(HTML.toHTML(out.get(i), Mode.TERSE)));
         ...
         case VERBOSE: {
            ...
            for(int i = 0; i < mSteps.size(); ++i) {
               ...
               for(final Object outtag : func.getOutputTags()) {
                  if(!valTags.containsKey(outtag)) {
                     ...
                     report.addHTML(HTML.error("Output error in Step " + Integer.toString(i + 1) + ": Attempting to redefine "
                           + HTML.toHTML(outtag, Mode.TERSE) + "."));
                  }
               }
            }
            report.addSubHeader("Input Utilization Table");
            {
               ...
               for(final Object tag : getInputTags())
                  tbl.addRow(Table.td(HTML.toHTML(tag, Mode.TERSE)), usage.containsKey(tag) ? Table.td(usage.get(tag))
                        : Table.td("Missing"));
               report.add(tbl);
            }
      ...
      for(int step = 0; step < mSteps.size(); ++step) {
         ...
         if(mFinalOutputs != null) {
            ...
            for(int i = 0; i < ovals.getDimension(); ++i) {
               assert !valTags.contains(fout.get(i)) : fout.get(i) + " computed in Step " + i + " has previously been defined.";
