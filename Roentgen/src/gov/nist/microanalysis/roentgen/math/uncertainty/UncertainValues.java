...
public class UncertainValues
   ...
   /**
    * Checks that the covariance matrix is m x m, the variances are
    * non-negative, the covariances are bounded correlation coefficients between
    * -1<r<1.
    *
    * @param m
    * @param covar
   ...
   /**
    * Constructs a UncertainValues object based on the specified tags, values
   ...
   /**
    * Constructs a UncertainValues object based on the specified tags, values
    * and variances array. "Heteroskedastic-case"
    *
    * @param tags List&lt;Object&gt; A list of objects implementing hashCode()
    *           and equals().
    * @param vals {@link RealVector}
   ...
   /**
    * Constructs a UncertainValues object based on the specified tags, values
    * and variances. "Homoskedastic-case"
    *
    * @param tags List&lt;Object&gt; A list of objects implementing hashCode()
    *           and equals().
    * @param vals {@link RealVector}
   ...
   public static UncertainValues build(final List<? extends Object> tags, final UncertainValues... uvs)
      ...
      for(final Object tag : tags) {
         ...
         if(count > 1)
            throw new ArgumentException("The tag " + tag + " is muliply defined in one of the input UncertainValues.");
      }
   ...
   /**
    * Return an {@link UncertainValues} object with the same dimension and
    * values as input except all the covariances except those associated with
    * tag are zeroed.
    *
    * @param tag
    * @param input
   ...
   public static UncertainValues propagate(final NamedMultivariateJacobianFunction nmjf, final UncertainValues vals) {
      ...
      return new UncertainValues(nmjf.getOutputTags(), eval.getFirst(), jac.multiply(vals.getCovariances().multiply(jac.transpose())));
   }
   ...
   public static UncertainValues propagateMC(final NamedMultivariateJacobianFunction nmjf, final UncertainValues vals, final int nEvals) {
      final MultivariateNormalDistribution mnd = new MultivariateNormalDistribution(vals.mValues.toArray(), vals.mCovariance.getData());
   ...
   public static UncertainValues implicit(final NamedMultivariateJacobianFunction jY, final NamedMultivariateJacobianFunction jX, UncertainValues uv) {
      ...
      // 1. Form the Cholesky factor Rx of Vx, i.e., the upper triangular matrix such that RxT.Rx = Vx.
      final CholeskyDecomposition chyd = new CholeskyDecomposition(uv.getCovariances());
      final RealMatrix Rx=chyd.getL();
      // 2. Factor Jx as the product Jx = Qx.Ux, where Qx is an orthogonal matrix and Ux is upper triangular.
      final QRDecomposition qrd = new QRDecomposition(jXe.getSecond());
      final RealMatrix Qx = qrd.getQ(), Ux = qrd.getR();
      // 3. Factor Jy as the product Jy = Ly.Uy, where Ly is lower triangular and Uy is upper triangular.
   ...
   /**
    * Returns the value associated with the entry associated with tag if the tag
    * is associated with a value or returns defVal otherwise. Useful for a
    ...
    * tag. The value is the same as getEntry(...) and the uncertainty is the
   ...
   /**
    * Returns an UncertainValue for the quantity assoicated with the specified
    * tag (if defined). The value is the same as getEntry(...) and the
    * uncertainty is the on-diagonal covariance matrix entry associated with
    * that entry. If the tag isn't defined, then the value defVal will be
    * returned.
    *
    * @param tag
    * @param defVal
    ...
    * @param tag1 Object
    * @param tag2 Object
    * @return double The covariance associated with tags tag1 and tag2 or 0 if
    *         one or both tags unknown
    */
   ...
   /**
    * Returns the value and the associated uncertainty as an
    * {@link UncertainValue} object.
    *
    * @param tag
   ...
   /**
    * Provides a mechanism to convert this {@link UncertainValues} object to
      ...
      switch(mode) {
         case TERSE: {
            ...
            for(final Object rowTag : mTags) {
               ...
               vals.add(Table.tdc(nf.formatHTML(getEntry(rowTag)) + "&pm;" + nf.formatHTML(Math.sqrt(getVariance(rowTag)))));
            }
            table.addRow(header);
            table.addRow(vals);
            ...
            for(int r = 0; r < mTags.size(); ++r) {
               ...
               for(final Object colTag : mTags) {
                  ...
                  if((rowTag == colTag) || (mode != Mode.VERBOSE))
                     row.add(Table.tdc("(" + nf.formatHTML(Math.sqrt(c)) + ")<sup>2</sup>"));
                  else {
                     ...
                     if(cc == 0.0)
                        ...
                        row.add(Table.tdc((cc < 0.0 ? "-" : "") + bnf2.formatHTML(Math.sqrt(Math.abs(cc)))
   ...
   public BufferedImage asCovarianceBitmap(final int dim, final IValueToColor sigma, final IValueToColor corr) {
      ...
      for(int r = 0; r < getDimension(); ++r) {
         ...
         for(int c = r + 1; c < getDimension(); ++c) {
            final double rr = mCovariance.getEntry(r, c) / (Math.sqrt(mCovariance.getEntry(r, r) * mCovariance.getEntry(c, c)));
            if(!Double.isNaN(rr)) {
      ...
      final BufferedImage bi = new BufferedImage(dim * getDimension(), dim * getDimension(), BufferedImage.TYPE_3BYTE_BGR);
   ...
   /**
    * <p>
    * Copies those entries tagged in <i>from</i> that are also tagged in
   ...
   /**
    * Sets the covariance value associated with the specified values (both i,j
    * and j,i)
    *
    * @param tag1
    * @param tag2
   ...
   /**
    * Checks the values and covariances to determine whether any are equivalent
    * to NaN.
    *
   ...
   /**
    * Build a new UncertainValues from this one in which the tags have been
    * sorted into an order determined by the specified {@link Comparator}.
    *
    * @param compare
    * @return {@link UncertainValues} A new instance with the same data
    *         reordered.
    */
   ...
   /**
    * Build a new UncertainValues from this one in which the tags have been
    * sorted into alphabetical order by tag.toString().
    *
    * @return {@link UncertainValues} A new instance with the same data
    *         reordered.
    */
