 ...
 * @version $Rev: $
 */
public class Histogram implements IToHTML {
	 ...
	 * ends define by binMins. The max end of the top bin is defined by max.
	 *
	 * @param binMins
	 * @param max
	 */
	 ...
	 * DescriptiveStatistics object with approximately the specified number of bins.
	 * The algorithm attempts to chose reasonable min and max limits based on the
	 * data.
	 *
	...
	public String toHTML(
		...
		if ((mode == Mode.NORMAL) || (mode == Mode.VERBOSE)) {
			...
			for (int i = -1; i < (binCount() + 1); ++i) {
				final List<String> row = Arrays.asList(Integer.toString(i), HTML.escape(binName(i, bnf)),
						i >= 0 ? bnf.format(minValue(i)) : "-&infin;",
						i < binCount() ? bnf.format(maxValue(i)) : "+&infin;", Integer.toString(counts(i)));
				items.add(row);
			}
