...
public class EMSAReader
   ...
   @Override
   public EDSSpectrum read(final InputStream fis, final int i)
         throws IOException {
      if(i > 1)
         return null;
      ...
      try (final BufferedReader br = new BufferedReader(rd)) {
         ...
         final String title = props.containsKey("#TITLE") ? props.get("#TITLE") : "Spectrum " + Arrays.hashCode(data);
   ...
   public boolean matches(final InputStream fis) {
      try (final InputStreamReader isr = new InputStreamReader(fis, Charset.forName("US-ASCII"))) {
         try (final BufferedReader br = new BufferedReader(isr)) {
            ...
            while(line.length() == 0)
               line = br.readLine().trim();
            final boolean res = line.substring(0, 7).equalsIgnoreCase("#FORMAT");
            return res;
         }
   ...
   public static class EMSAFileReaderFactory
      implements
      ISpectrumReaderFactory {
