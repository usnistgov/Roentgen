...
public class HalfUpFormat extends DecimalFormat {
	 ...
	 */
	public HalfUpFormat(final boolean spaceGrouping) {
		super();
		if (spaceGrouping) {
	 ...
	 * @param pattern
	 */
	public HalfUpFormat(final String pattern) {
		this(pattern, false);
	}
	 ...
	 * @param pattern
	 */
	public HalfUpFormat(final String pattern, final boolean spaceGrouping) {
		super(pattern);
		if (spaceGrouping) {
			final DecimalFormatSymbols dfs = DecimalFormatSymbols.getInstance();
			dfs.setGroupingSeparator(MEDIUM_MATHEMATICAL_SPACE);
	 ...
	 * @param symbols
	 */
	public HalfUpFormat(final String pattern, final DecimalFormatSymbols symbols) {
		super(pattern, symbols);
		setRoundingMode(RoundingMode.HALF_UP);
	}

	/**
	 * Adaptively format the number such that the specified number is output using
	 * HalfUpFormating with the specified precision. The format is adaptively built
	 * to ensure that only precision significant digits are displayed and that the
	 * output mode switches to scientific when the absolute value is larger than
	 * sciRange or the absolute value is less than 1.0/sciRange.
	 *
	 ...
	 */
	static public String adaptiveFormat(final double number, int precision, double sciRange) {
