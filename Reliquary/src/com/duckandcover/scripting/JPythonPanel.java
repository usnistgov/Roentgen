 ...
 * @version $Rev: 309 $
 */
public class JPythonPanel extends JPanel {
	...
	private static final class OpenPyScriptListener implements HyperlinkListener {
		@Override
	...
	private static final class OpenHTMLListener implements HyperlinkListener {
		@Override
		public void hyperlinkUpdate(
				final HyperlinkEvent e
		) {
	...
	private void init() {
		...
		/*
		 * final Configuration c = DefaultSyntaxKit.getConfig(DefaultSyntaxKit.class);
		 * for (final Map.Entry<String, String> me : c.entrySet())
	...
	private void addEditorKeyEvents() {
		...
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.CTRL_DOWN_MASK), "prev-history");
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, InputEvent.CTRL_DOWN_MASK), "next-history");
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, InputEvent.CTRL_DOWN_MASK), "execute-script");
	}
	 ...
	 */
	public void append(
			final String html, final boolean scrollTo
	) {
		try {
	...
	/**
	 * Writes an image to the report directory and then returns the name of the file
	 * in the report path. Add this name to the user generated HTML code.
	 *
	 * @param img
	 * @return String
	 * @throws IOException
