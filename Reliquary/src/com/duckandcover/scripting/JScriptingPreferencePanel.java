...
public class JScriptingPreferencePanel extends JPreferencePanel {
    ...
    private final JTextField jTextField_ClassPath = new JTextField();
	...
	yy += 2;
	pb.add(new JLabel("Jython CLASSPATH"), CC.xy(2, yy), jTextField_ClassPath, CC.xy(4, yy));
	jTextField_ClassPath.setToolTipText("A list of paths to JAR library files separated by the ';' character");
	yy += 2;
	...
	prefs.put(CLASSPATH, jTextField_ClassPath.getText());
	...
	try {
	    parseClassPathItems(jTextField_ClassPath.getText());
	} catch (final FileNotFoundException e) {
	    JErrorDialog.createErrorMessage(this.getParent(), "Bad CLASSPATH error", e);
	    return false;
	}
	return true;
    }
    ...
    private String[] parseClassPathItems(final String cp) throws FileNotFoundException {
	...
	jTextField_ClassPath.setText(getClassPath());
    ...
    /**
     * Returns the classpath used to initialize the scripting environment to
     * permit custom JARs to be loaded.
     *
     * @return String
     */
