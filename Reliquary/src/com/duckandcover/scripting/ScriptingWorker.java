...
/**
 * <p>
 * A SwingWorker thread which runs a scripting engine and provides mechanisms
 * for queuing tasks on the scripting engine.
 * </p>
 *
 * @author Nicholas W. M. Ritchie
 * @version 1.0
 */
public class ScriptingWorker extends SwingWorker<Object, ScriptOutput> {
	...
	abstract private class PyOutStream extends Writer {
	...
	/**
	 * Excute a single command string using the scripting engine. @param cmd @throws
	 * InterruptedException
	 */
	...
	/**
	 * Excute the commands in a file using the scripting engine. @param file @throws
	 * InterruptedException
	 */
	...
	private void execNow(final InputStream inputStream) throws IOException {
	 ...
	 * output go through the stdout and stderr mechanism. Error output is generated
	...
	protected Object doInBackground() throws Exception {
		...
		try {
			...
			try {
				...
				publish(new ScriptOutput.Error(t1.toString()));
			}
			try {
				...
				Reliquary.getLogger().log(Level.WARNING,
						"Error executing script: Internal startUp.py script from resource.", e1);
			}
			if (mStartup != null) {
				...
				if (obj instanceof File) {
					...
					try {
						...
						if (mScripter.get(TERMINATED).__nonzero__())
							publish(new ScriptOutput.Error(
									"Warning: Script execution may have been terminated prematurely."));
					} catch (final Throwable e) {
						publish(new ScriptOutput.Error(e.toString()));
					}
					...
					if (elapse > (10 * 1000)) // 10 s
						publish(new ScriptOutput.HTML(
								HTML.br() + HTML.p(HTML.i("Elapse:") + "&nbsp;" + elapseToString(elapse) + HTML.br())));
					...
					if (cmds.length() > 0) {
						...
						try {
							...
							if (mScripter.get(TERMINATED).__nonzero__())
								publish(new ScriptOutput.Error(
										"Warning: Command execution may have been terminated prematurely."));
						} catch (final Throwable e) {
							publish(new ScriptOutput.Error(e.toString()));
						}
					}
